// natch.js — robust match-by-id loader (keeps your displayMatch unchanged)
(function () {
  // ----- auto-init safely whether script is in <head> or at the end -----
  function boot() {
    const containers = document.querySelectorAll(".match-widget-container, .football-widget");
    if (!containers.length) return;

    containers.forEach(container => {
      // Accept both attribute spellings
      const league =
        container.dataset.league || container.getAttribute("data-league");
      const fixtureId =
        container.dataset.id ||
        container.getAttribute("data-id") ||
        container.dataset.fixture ||
        container.getAttribute("data-fixture");

      if (!league || !fixtureId) {
        container.innerHTML = "Missing league or match id";
        return;
      }
  ...
  // ----- fetch helpers: try ?id=, then ?fixture=, then ?event= -----
  async function fetchById(league, fixtureId) {
    const base = "https://love-weld-nine.vercel.app/api/espn";
    const urls = [
      `${base}?league=${encodeURIComponent(league)}&id=${encodeURIComponent(fixtureId)}`,
      `${base}?league=${encodeURIComponent(league)}&fixture=${encodeURIComponent(fixtureId)}`,
      `${base}?league=${encodeURIComponent(league)}&event=${encodeURIComponent(fixtureId)}`
    ];
    ...
    for (const url of urls) {
      try {
        ...
        // two common shapes: {events:[...] } or {event:{...}}
        if (data?.events?.length) return { match: data.events[0], url };
        if (data?.event) return { match: data.event, url };
      } catch (e) {
  ...
  // ----- main loader (uses your existing displayMatch) -----
  async function loadMatch(container, league, fixtureId) {
    const storageKey = `match_${fixtureId}`;

    try {
      const result = await fetchById(league, fixtureId);
      if (!result || !result.match) {
        return loadFromStorage("No matches available");
      }
    ...
    function loadFromStorage(fallbackMsg) {
      try {
        ...
        if (stored) {
          displayMatch(JSON.parse(stored), true);
          return;
        }
      } catch (_) {}
    ...
    // ---------- KEEP YOUR EXISTING RENDERER HERE ----------
    // Paste your full displayMatch(match, fromStorage=false) function below,
    // unchanged. Everything else is already wired for match-id loading.
    function displayMatch(match, fromStorage = false) {
      // ... your original display code exactly as you wrote it ...
